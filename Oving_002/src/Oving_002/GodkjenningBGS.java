...
public class GodkjenningBGS {
  ...
  private void registrerNyStudent() {
    ...
    if (oversikt.regStudent(navnNyStud)) {
    ...
    private void registrerOppgaver(String studNavn) {
      ...
      do { // gjentar inntil registrering aksepteres av objektet oversikt
        try {
          ...
          oversikt.økAntOppgaver(studNavn, antOppgØkning);  // kan ikke returnere false, pga navn alltid gyldig
      ...
      melding = "Totalt antall oppgaver registrert på " + studNavn + " er " + oversikt.finnAntOppgStudent(studNavn) + ".";
